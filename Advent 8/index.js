const allDirections = [[1,0], [0,1], [-1,0], [0,-1]];
const fs = require("fs");
...
function solve(){
    ...
    for(let i = 1; i < matrix.length - 1; i++){
        for(let j = 1; j < matrix[i].length - 1; j++){
            let results = allDirections.map((direction) => isVisibleBy(matrix, [i,j], direction)).reduce((a,b) => [a[0] || b[0], a[1] * b[1]]);
            if(results[0]){
                visibles++;
            }
            if(results[1] > maxScore){
                maxScore = results[1];
            }
        }
    }
...
function isVisibleBy(matrix, initialPos, direction) {
